...
sidebar: sidebar 
keywords:  
summary: Saiba mais sobre os recursos e funcionalidades que foram removidos ou descontinuados no StorageGRID 12.1.0. Antes de atualizar, verifique se algum recurso removido ou descontinuado afeta seus aplicativos cliente ou a configuração da grade. 
---
= Recursos e funcionalidades removidos ou descontinuados no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Saiba mais sobre os recursos e funcionalidades que foram removidos ou descontinuados no StorageGRID 12.1.0. Antes de atualizar, verifique se algum recurso removido ou descontinuado afeta seus aplicativos cliente ou a configuração da grade.
...
Obsoleto:: Você *não deve* usar esse recurso em novos ambientes de produção. Ambientes de produção existentes podem continuar usando o recurso.
...
| Recurso | Obsoleto | Fim da vida | Removido | Links para documentação anterior 
 ...
 a| 
Configuração global de SSE para criptografia AES-128
 a| 
12,1
 a| 
12,2
...
12,3
 a| 
A partir do StorageGRID 12.1.0, o suporte para criptografia AES-128 de novos dados de objetos armazenados em repouso está obsoleto. O suporte continua para a recuperação de objetos armazenados que foram originalmente criptografados com AES-128.



 a| 
Alarmes, legado (_não Alertas_)
 a| 
11,7
 a| 
11,8
...
. Desative todos os nós de arquivamento. Consulte  https://docs.netapp.com/us-en/storagegrid-118/maintain/grid-node-decommissioning.html["Desativação do nó de grade (StorageGRID 11,8 doc site)"^].
. Remova todas as referências ao nó de arquivamento dos pools de armazenamento e das políticas ILM. Consulte  https://kb.netapp.com/hybrid/StorageGRID/Maintenance/StorageGRID_11.9_software_upgrade_resolution_guide["Base de dados de Conhecimento da NetApp: Guia de resolução de atualização do software StorageGRID 11,9"^].
...
12,1
 a| 
https://docs.netapp.com/us-en/storagegrid-118/monitor/managing-alarms.html#configure-notifications-for-alarms-legacy-system["Configurar notificações para alarmes (sistema legado) (StorageGRID 11.8)"^]
