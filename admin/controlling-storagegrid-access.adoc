...
summary: 'Você controla quem pode acessar o StorageGRID e quais tarefas os usuários podem executar criando ou importando grupos e usuários e atribuindo permissões a cada grupo. Opcionalmente, você pode ativar o logon único (SSO), criar certificados de cliente e alterar senhas de grade.' 
---
...
[role="lead"]
Você controla quem pode acessar StorageGRID e quais tarefas os usuários podem executar, criando ou importando grupos e usuários e atribuindo permissões a cada grupo. Opcionalmente, você pode habilitar o logon único (SSO), criar certificados de cliente, alterar senhas do grid e usar a verificação multi-admin.
...
O sistema StorageGRID suporta autenticação única (SSO) usando o padrão Security Assertion Markup Language 2.0 (SAML 2.0) ou OpenID Connect (OIDC). Após você link:how-sso-works.html["Configurar e ativar SSO"], todos os usuários devem ser autenticados por um provedor de identidade externo antes de acessar o Grid Manager, o Tenant Manager, a API de gerenciamento do Grid ou a API de gerenciamento do Tenant. Usuários locais não podem fazer login no StorageGRID.
...
== Use a verificação multi-admin

Você pode usar a verificação multiadministrador (MAV) para garantir que determinadas operações do StorageGRID só possam ser executadas após a aprovação de administradores designados. A MAV impede que administradores comprometidos, maliciosos ou inexperientes façam alterações indesejáveis ou excluam dados.
