...
[role="lead"]
Se você tem link:../admin/how-sso-works.html["Logon único configurado e habilitado (SSO)"] e usa o PingFederate como provedor SSO, você deve emitir uma série de solicitações de API para obter um token de autenticação válido para a API de Gerenciamento de Grade ou para a API de Gerenciamento do locatário.
