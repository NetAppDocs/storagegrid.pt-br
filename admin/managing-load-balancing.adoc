...
Se você planeja usar solicitações no estilo de hospedagem virtual do S3, o certificado também deve incluir uma entrada de *Nome Alternativo* para cada link:../admin/configuring-s3-api-endpoint-domain-names.html["Nome de domínio do endpoint S3"] que você configurou, incluindo quaisquer nomes curinga. Por exemplo:
