...
---
= Considerações e requisitos do assistente de instalação do StorageGRID S3
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
--
Se você planeja usar solicitações no estilo virtual-hosted do S3, você deve ter link:../admin/configuring-s3-api-endpoint-domain-names.html["Configurados S3 nomes de domínio de endpoint"]. O uso de solicitações no estilo virtual-hosted é recomendado.
...
--
Configure todas as conexões de federação de grade:: Se você deseja permitir que o locatário do S3 sincronize dados da conta e replique objetos do bucket para outra grid usando uma conexão de federação de grid, confirme o seguinte antes de iniciar o assistente:
+
--
