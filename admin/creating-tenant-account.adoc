...
* Se você deseja permitir que um locatário do S3 sincronize dados de conta e replique objetos de bucket para outra grid usando uma conexão de federação de grid:
+
...
Conexão de federação de grade
 a| 
O tenant pode usar conexões de federação de grid, que:

** Faz com que esta conta de locatário e todos os grupos de locatário, usuários e chaves de acesso S3 sejam sincronizados entre esta grid e as outras grids nas conexões selecionadas.
+

TIP: Se necessário por motivos de segurança ou conformidade, você pode desmarcar a opção *Enable access key sync*.
...
. Se você selecionou a permissão *Usar conexão de federação de grid* para o tenant, confirme se um tenant idêntico foi replicado para o outro grid na conexão. Os tenants em ambos os grids terão o mesmo ID de conta de 20 dígitos, nome, descrição, cota e permissões.
+

NOTE: Se você vir uma mensagem de erro indicando que a conta do locatário foi criada nesta grid, mas não foi sincronizada com as outras grids, consulte as instruções em link:grid-federation-troubleshoot.html["Solucionar erros de federação de grade"].
...
Siga as instruções em link:../tenant/index.html["Use uma conta de locatário"] para gerenciar grupos de locatários e usuários, chaves de acesso S3, buckets, serviços de plataforma, sincronização de contas e replicação entre grids.
