...
summary: 'Antes de usar um servidor de gerenciamento de chaves (KMS) para proteger os dados do StorageGRID nos nós do appliance, você precisa configurar um ou mais servidores KMS e habilitar a criptografia para os nós do appliance. Quando essas duas tarefas de configuração forem concluídas, o processo de gerenciamento de chaves ocorrerá automaticamente.' 
---
...
image:kms_configuration_overview.png["Fluxo de trabalho de configuração DE KMS, que é descrito no texto seguinte"]
