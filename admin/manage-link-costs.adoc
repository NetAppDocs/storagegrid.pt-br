...
---
= Gerencie os custos de link no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Os custos de link são usados pelo serviço Load Balancer nos nós de administração e nos nós de gateway para direcionar as conexões dos clientes. Consulte link:../admin/managing-load-balancing.html["Considerações para balanceamento de carga"].


O diagrama mostra uma grade de três sites com custos de enlace configurados entre os sites:

image:link_costs.png["Diagrama conceitual para custos de ligação entre data centers"]
...
. Para cada site listado na coluna Custo do Site/Link, insira um valor de custo de 0 a 100 para os outros sites.
+
Não é possível alterar o custo do link se a origem for a mesma do destino.

. Descartar ou salvar alterações:
+
** Para descartar as alterações, selecione *Limpar alterações*.
** Para salvar as alterações, selecione *Salvar*.
