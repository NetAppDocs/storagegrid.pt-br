...
---
= Compare a replicação entre grades e a replicação CloudMirror no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Adicione novas contas de locatário, que são sincronizadas automaticamente com a outra grid.
. Adicione novos grupos de locatários e usuários, que também são sincronizados.
