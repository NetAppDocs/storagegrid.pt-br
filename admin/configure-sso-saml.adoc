...
summary: 'Você pode seguir o assistente de configuração de SSO para SAML e entrar no modo sandbox para configurar e testar o single sign-on (SSO) antes de habilitá-lo para todos os usuários do StorageGRID. Após o SSO ser habilitado, você pode retornar ao modo sandbox quando necessário para alterar ou testar novamente a configuração.' 
---
...
== [[enter-sandbox-mode]]Etapa 2: Forneça relações de confiança de terceiros, aplicativos corporativos ou conexões SP
...
. Siga os passos descritos em link:../admin/creating-enterprise-application-entra-id-saml.html["Crie aplicativos corporativos no Entra ID"] para criar uma aplicação empresarial para cada Admin Node listado na tabela.
...
+
Para obter instruções, acesse link:../admin/creating-relying-party-trusts-in-ad-fs-saml.html["Criar confiança de parte confiável no AD FS"].
...
. Siga os passos em link:../admin/creating-enterprise-application-entra-id-saml.html["Crie aplicativos corporativos no Entra ID"] para fazer upload do arquivo de metadados SAML de cada Admin Node no respectivo aplicativo corporativo Entra ID.
...
. link:../admin/creating-sp-connection-ping-saml.html["Crie uma ou mais conexões de provedor de serviços (SP) para o StorageGRID"]. Use o ID de conexão do SP para cada Nó de Administração (mostrado na tabela na página Configurar SSO) e os metadados SAML que você baixou para esse Nó de Administração.
+
...
== [[test-sso]]Etapa 4: testar configuração
...
== [[enable-sso-saml]]Etapa 5: habilitar o login único
...
Active Directory
 a| 
link:creating-relying-party-trusts-in-ad-fs-saml.html["Crie um grupo de aplicativos para cada Admin Node"]
...
ID de entrada
 a| 
link:creating-enterprise-application-entra-id-saml.html["Crie um aplicativo para cada Admin Node"]
...
PingFederate
 a| 
link:creating-sp-connection-ping-saml.html["Crie uma conexão de provedor de serviços para cada Admin Node"]
