...
summary: Você pode criar conexões de federação de grid entre sistemas StorageGRID se desejar sincronizar detalhes de locatários e replicar dados de objetos. 
---
...
[role="lead"]
Você pode criar conexões de federação de grid entre sistemas StorageGRID se desejar sincronizar detalhes de locatários e replicar dados de objetos.
...
TIP: Opcionalmente, você pode link:../admin/grid-federation-automate-connections.html["Automatize a criação, edição e manutenção de conexões de federação de grid"].

image:grid_federation_create_connection.png["visão geral gráfica das etapas para criar uma conexão em duas grades"]
...
. Insira um nome de conexão, que seja um nome exclusivo para ajudar você a reconhecer esta conexão. Por exemplo,  `Grid 1-Grid 2`.
. Se você quiser link:../admin/configure-global-namespace.html["configurar um namespace global"], selecione *Ativar namespace global*.
. Selecione *continuar*.
. Insira os detalhes da grid, conforme segue:
...
* link:grid-federation-troubleshoot.html["Solucionar problemas da conexão"], incluindo a resolução de quaisquer alertas e erros relacionados à sincronização de contas e à replicação entre grids.
