...
summary: A replicação entre grids é a replicação automática de objetos entre buckets S3 selecionados em dois sistemas StorageGRID conectados por uma conexão de federação de grids. A sincronização de contas é necessária para a replicação entre grids. 
---
...
[role="lead"]
A replicação entre grades é a replicação automática de objetos entre buckets S3 selecionados em dois sistemas StorageGRID conectados em uma link:grid-federation-overview.html["conexão de federação de grade"]. link:grid-federation-what-is-account-sync.html["Sincronização de contas"]é necessário para a replicação entre grades.
...
image:grid_federation_cgr_workflow.png["Fluxo de trabalho de replicação entre grade"]
...
Se uma conta de locatário tiver a permissão *Usar conexão de federação de grid* para usar uma ou mais link:grid-federation-overview.html["conexões de federação de grade"], um usuário de locatário com permissão de acesso Root pode criar buckets ("source" e "destination" buckets) nas contas de locatário correspondentes em cada grid. Esses buckets:

* Podem ter nomes diferentes uns dos outros
* Pode ter regiões diferentes
* Deve ter o controle de versão habilitado
* Deve possuir uma das seguintes configurações de S3 Object Lock:
+
** origem: *desativada*, destino: *desativada*
** origem: *ativada*, destino: *ativada*
** origem: *desativada*, destino: *ativada*




Depois de criar os dois buckets, você pode configurar a replicação entre grades para um ou ambos os buckets.
...
image:grid_federation_cross_grid_replication_one_direction.png["imagem mostrando a conexão de federação de grade em uma direção"]
...
image:grid_federation_cross_grid_replication.png["imagem mostrando replicação em uma direção em comparação com a replicação em ambas as direções"]
...
image:grid_federation_cross_grid_replication_delete.png["imagem que mostra a eliminação do cliente replicado em ambas as grelhas"]
...
CAUTION: Você pode habilitar o Bloqueio de Objetos S3 em buckets existentes, o que pode levar a uma configuração incorreta posteriormente.


NOTE: Quando o S3 Object Lock estiver ativado no bucket de origem, certifique-se de que ele também esteja ativado no bucket de destino. Se o S3 Object Lock estiver desativado no bucket de destino, ocorrerá um erro durante a replicação. Consulte link:../admin/grid-federation-troubleshoot.html#cross-grid-replication-alerts-and-errors["Alertas e erros de replicação entre redes"].
