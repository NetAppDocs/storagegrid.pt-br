...
---
= Crie relações de confiança com terceiros confiáveis no AD FS para SAML no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Você deve usar o Active Directory Federation Services (AD FS) para criar uma relação de confiança de terceiros para cada nó Admin para SAML em seu sistema. Você pode criar relações de confiança de terceiros usando comandos PowerShell, importando metadados SAML do StorageGRID ou inserindo os dados manualmente.

.Antes de começar
* Você configurou o logon único para o StorageGRID e selecionou *AD FS* como o tipo SSO.
* Você tem link:../admin/configure-sso-saml.html["entrou no modo sandbox"] no Grid Manager.
...
. Ao concluir, retorne ao StorageGRID e link:../admin/configure-sso-saml.html#test-sso["testar todos os trusts de terceiros confiáveis"] para confirmar que estão configurados corretamente.
...
. Repita essas etapas para configurar uma confiança de parte confiável para todos os nós de administração no sistema StorageGRID.
. Ao concluir, retorne ao StorageGRID e link:../admin/configure-sso-saml.html#test-sso["testar todos os trusts de terceiros confiáveis"] para confirmar que estão configuradas corretamente.
