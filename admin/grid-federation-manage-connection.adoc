...
NOTE: Enquanto a conexão estiver sendo editada, as solicitações de sincronização de conta ou replicação entre grades continuarão usando as configurações de conexão existentes. Você deve editar uma conexão por vez e enviar um arquivo de verificação para a grade remota para cada conexão. As edições são aplicadas após serem enviadas para as grades remotas.
...
image:grid_federation_remove_connection.png["etapas para remover a conexão de federação de grade"]
