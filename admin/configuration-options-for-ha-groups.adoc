...
---
= Opções de configuração para grupos de alta disponibilidade do StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Os diagramas a seguir fornecem exemplos de diferentes maneiras de configurar grupos de alta disponibilidade (HA). Cada opção tem vantagens e desvantagens.

Nos diagramas, azul indica a interface principal no grupo HA e amarelo indica a interface de backup no grupo HA.

image:high_availability_examples.png["HA exemplos"]
