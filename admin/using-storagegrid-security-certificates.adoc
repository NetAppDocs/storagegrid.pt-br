...
O StorageGRID tem uma autoridade de certificação (CA) integrada que gera um certificado interno do Grid CA durante a instalação do sistema.  O certificado Grid CA é usado, por padrão, para proteger o tráfego interno do StorageGRID .  Uma autoridade de certificação (CA) externa pode emitir certificados personalizados que são totalmente compatíveis com as políticas de segurança de informações da sua organização.

Use o certificado Grid CA para ambientes que não sejam de produção.  Para produção, use certificados personalizados assinados por uma autoridade de certificação externa.  Conexões não seguras sem certificado são suportadas, mas não recomendadas.
...
*** link:../monitor/configure-log-management.html#use-external-syslog-server["Use um certificado para um servidor syslog externo"]
...
*** link:creating-relying-party-trusts-in-ad-fs-saml.html#create-a-relying-party-trust-manually["Especifique manualmente um certificado SSO para uma confiança de parte dependente"]
...
Servidor
 a| 
Autentica a conexão de um StorageGRID Cloud Storage Pool para um local de armazenamento externo, como Amazon S3 Glacier ou Microsoft Azure Blob Storage. É necessário um certificado diferente para cada tipo de provedor de nuvem.
 a| 
...
*Configuração* > *Monitoramento* > *Servidor de auditoria e syslog*
 a| 
link:../monitor/configure-log-management.html#use-external-syslog-server["Use um servidor syslog externo"]
