...
---
= Gerencie o armazenamento de metadados de objetos no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:metadata_space_across_sites.png["Espaço de metadados entre sites"]
...
A configuração padrão para uma nova instalação do StorageGRID 12.1 ou superior, na qual cada nó de armazenamento possui 128 GB ou mais de RAM, é de 20.000 GB (20 TB).
...
+
image:storage_used_object_metadata_actual_reserved.png["Armazenamento usado - metadados Objeto - Real reservado"]



Na captura de tela, o valor reservado real é de 20 TB. Esta captura de tela é de um Storage Node grande em uma nova instalação do StorageGRID 12.1. O volume 0 tem pelo menos 20 TB, então usa a configuração padrão de espaço reservado para metadados em todo o sistema.
...
Suponha que você instale um novo sistema StorageGRID usando a versão 12.1 ou posterior. Para este exemplo, suponha que cada Storage Node tenha mais de 128 GB de RAM e que o volume 0 do Storage Node 1 (SN1) tenha 6 TB. Com base nesses valores:

* O *espaço reservado de metadados* em todo o sistema está definido como 20 TB. (Este é o valor padrão para uma nova instalação do StorageGRID 12.1 ou superior se cada Storage Node tiver mais de 128 GB de RAM.)
* O espaço *realmente reservado* para metadados no SN1 é de 6 TB. (Todo o volume está reservado porque o volume 0 é menor que a configuração de *Espaço reservado para metadados*.)
...
image:metadata_allowed_space_volume_0.png["Metadados permitiram volume de espaço 0"]
...
&Lt; 4 TB
 a| 
75% do espaço reservado real para metadados, até um máximo de 1,32 TB
 a| 
75% do espaço reservado real para metadados, até um máximo de 1,98 TB
...
&Gt; 4 TB
 a| 
(espaço reservado para metadados − 1 TB) × 75%, até um máximo de 1,32 TB
 a| 
(espaço reservado para metadados − 1 TB) × 75%, até um máximo de 7,92 TB
...
+
image:storage_used_object_metadata_allowed.png["Armazenamento usado - metadados Objeto - permitido"]



Na captura de tela, o valor *Permitido* é 7,92 TB, que é o valor máximo para um Storage Node cujo espaço reservado para metadados é superior a 4 TB.
...
* O *espaço reservado de metadados* em todo o sistema está definido como 20 TB. (Este é o valor padrão para uma nova instalação do StorageGRID 12.1 ou superior se cada Storage Node tiver mais de 128 GB de RAM.)
* O espaço *realmente reservado* para metadados no SN1 é de 6 TB. (Todo o volume está reservado porque o volume 0 é menor que a configuração de *Espaço reservado para metadados*.)
* O espaço *permitido* para metadados no SN1 é de 3,75 TB, com base no cálculo mostrado em <<table-allowed-space-for-metadata,tabela para espaço permitido para metadados>>: (espaço reservado para metadados − 1 TB) × 75%, até um máximo de 7,92 TB.
...
image:metadata_space_three_storage_nodes.png["Espaço de metadados três nós de storage"]
