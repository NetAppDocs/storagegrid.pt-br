...
---
= Crie aplicativos corporativos no Entra ID para SAML no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Use o Entra ID para criar um aplicativo corporativo para cada nó de administração para SAML em seu sistema.

.Antes de começar
* Você começou a configurar o logon único para o StorageGRID e selecionou *Entra ID* como o tipo de SSO.
* Você tem link:../admin/configure-sso-saml.html#enter-sandbox-mode["entrou no modo sandbox"] no Grid Manager.
...
== Etapa 2: Crie aplicativos corporativos e salve a configuração do SSO do StorageGRID
...
== Etapa 3: Baixe os metadados SAML para cada Admin Node
...
== Etapa 4: Carregue os metadados SAML em cada aplicação empresarial
...
. link:../admin/configure-sso-saml.html#test-sso["Teste cada aplicação"].
