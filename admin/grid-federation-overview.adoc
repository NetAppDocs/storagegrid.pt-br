...
summary: Você pode usar a federação de grids para sincronizar tenants e replicar seus objetos entre vários sistemas StorageGRID para recuperação de desastres. 
---
= Saiba mais sobre a federação de grids StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Você pode usar a federação de grids para sincronizar tenants e replicar seus objetos entre vários sistemas StorageGRID para recuperação de desastres.
...
image:grid_federation_workflow.png["Fluxo de trabalho de conexão de federação de grade"]
...
+
Para obter detalhes sobre os requisitos de versão, consulte a  https://library.netapp.com/ecm/ecm_download_file/ECMLP3373065["Notas de lançamento"^]. Você precisa fazer login com sua NetApp conta ou criar uma conta para acessar as notas de versão.
...
* É necessário que exista pelo menos uma conexão de federação de grid antes de você poder usar link:grid-federation-what-is-account-sync.html["sincronização de contas"] ou link:grid-federation-what-is-cross-grid-replication.html["replicação entre grade"].
...
image:grid_federation_certificates.png["Certificados de federação de grade"]
