...
---
= Especifique as sub-redes da Grid Network para StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Selecione o sinal de mais ao lado da última entrada para adicionar uma entrada de rede adicional. Você deve especificar todas as sub-redes para todos os sites na Grid Network.
+
** Se você já implantou pelo menos um nó, selecione *Descobrir sub-redes da Grid Networks* para preencher automaticamente a lista de sub-redes da Grid Network com as sub-redes relatadas pelos nós da grid que se registraram no Grid Manager.
...
+
image:4_gmi_installer_grid_network_page.gif["Especifique sub-redes"]
