...
---
= Aprovar nós de grid pendentes para StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image:5_gmi_installer_grid_nodes_pending.gif["Esta imagem é explicada pelo texto circundante."]
...
.. Retorne à página inicial e selecione *Iniciar instalação*.
...
+
image:7_gmi_installer_grid_nodes_approved.gif["Esta imagem é explicada pelo texto circundante."]

. Repita estas etapas para cada nó de grade pendente que você deseja aprovar.
+
Você deve aprovar todos os nós que deseja na grid. No entanto, você pode retornar a esta página a qualquer momento antes de selecionar *Instalar* na página Summary. Você pode modificar as propriedades de um nó de grid aprovado selecionando seu botão de opção e selecionando *Editar*.

. Quando você terminar de aprovar os nós da grade, selecione *Avançar*.
