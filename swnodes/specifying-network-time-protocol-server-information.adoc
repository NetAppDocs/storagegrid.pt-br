...
---
= Especifique as informações do servidor Network Time Protocol para StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
https://learn.microsoft.com/en-us/troubleshoot/windows-server/active-directory/support-boundary-high-accuracy-time["Limite de suporte para tempo de alta precisão"^]
...
(Somente VMware) Execute verificações adicionais para VMware, como garantir que o hipervisor use a mesma fonte NTP que a máquina virtual e usar o VMTools para desabilitar a sincronização de tempo entre o hipervisor e as máquinas virtuais StorageGRID .
...
+
image:8_gmi_installer_ntp_page.gif["Esta imagem é explicada pelo texto circundante."]
