...
summary: Você pode usar a ferramenta VMware OVF para automatizar a implantação de nós de grid no StorageGRID. Você também pode automatizar a configuração do StorageGRID. 
---
= Automatize a implantação de nós de grid no StorageGRID com VMware vSphere
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
