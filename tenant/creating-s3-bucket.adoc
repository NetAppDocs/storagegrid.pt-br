...
* Caso vários locatários tenham até 5.000 buckets cada, cada Storage Node na grid deverá ter no mínimo 64 GB de RAM.
+

NOTE: O sistema StorageGRID suporta um máximo de 20.000 buckets para cada conta de tenant do S3. Cada grid pode ter um máximo de 100.000 buckets, incluindo link:../tenant/what-is-branch-bucket.html["baldes de galhos"]. Há também um limite para link:../tenant/manage-branch-buckets.html["profundidade do bucket de branch"].
...
Para mais informações, consulte a https://docs.aws.amazon.com/AmazonS3/latest/userguide/bucketnamingrules.html["Documentação da Amazon Web Services (AWS) sobre regras de nomenclatura de bucket"^].
