...
É possível configurar qualquer combinação de serviços de plataforma para um único bucket S3. Por exemplo, você pode configurar tanto o link:../tenant/understanding-cloudmirror-replication-service.html["Serviço CloudMirror"] quanto o link:../tenant/understanding-notifications-for-buckets.html["notificações"] em um bucket S3 do StorageGRID para espelhar objetos específicos no Amazon Simple Storage Service (S3), enquanto envia uma notificação sobre cada objeto para um aplicativo de monitoramento de terceiro, ajudando você a acompanhar seus gastos com a AWS.
...
Balde com bloqueio de objetos S3 ativado
 a| 
Se o bucket S3 de origem no StorageGRID tiver o S3 Object Lock ativado, você também deve ativar o S3 Object Lock para o bucket de destino com a retenção padrão configurada.

A replicação de _dados de objetos_ é compatível para buckets de origem e destino com o S3 Object Lock ativado. No entanto, os buckets de destino manterão suas próprias configurações de retenção de objetos e podem não oferecer conformidade ou versionamento.
