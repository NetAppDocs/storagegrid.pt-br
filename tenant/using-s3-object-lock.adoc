...
Quando a configuração global de S3 Object Lock está habilitada para um sistema StorageGRID, uma conta de locatário S3 pode criar buckets com ou sem S3 Object Lock habilitado. Quando a configuração permitir S3 Object Lock para buckets existentes está habilitada, uma conta de locatário S3 pode habilitar S3 Object Lock para um bucket existente. Se um bucket tiver S3 Object Lock habilitado, o versionamento do bucket é obrigatório e é habilitado automaticamente. Depois que uma conta de locatário S3 habilita S3 Object Lock para um bucket novo ou existente, ele não pode ser desabilitado.
...
+
Para obter detalhes sobre as configurações do objeto, consulte link:../s3/use-s3-api-for-s3-object-lock.html["Use a API REST do S3 para configurar o bloqueio de objetos do S3"].
...
Consulte link:../tenant/creating-s3-bucket.html["Crie um bucket do S3"] e link:../tenant/update-default-retention-settings.html["Atualização S3 retenção padrão bloqueio Objeto"].
...
Saiba mais sobrelink:../ilm/requirements-for-s3-object-lock.html#s3-object-lock-existing-buckets["Requisitos de bloqueio de objeto S3"]
