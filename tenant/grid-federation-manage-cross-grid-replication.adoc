...
summary: Se a sua conta de locatário recebeu a permissão Use grid federation connection no momento da criação, você pode usar a replicação entre grades para replicar automaticamente objetos entre buckets na grade de origem do locatário e buckets na grade de destino do locatário. A replicação entre grades pode ocorrer em uma ou ambas as direções. 
---
...
image:grid-federation-cgr-workflow.png["Fluxo de trabalho de replicação entre grade"]
...
+
image:grid-federation-cross-grid-replication-client-deletes.png["imagem que mostra a eliminação do cliente replicado em ambas as grelhas"]
