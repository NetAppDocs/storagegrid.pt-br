...
* A replicação só é compatível com buckets de origem ou destino que tenham o S3 Object Lock ativado para buckets Amazon S3 ou StorageGRID S3. Os cabeçalhos do object lock não são replicados para o objeto de destino.
...
* Se você habilitar o S3 Object Lock no bucket de destino após configurar a replicação CloudMirror, talvez seja necessário link:../tenant/editing-platform-services-endpoint.html["Salve novamente o endpoint"] para permitir que a replicação seja retomada.
