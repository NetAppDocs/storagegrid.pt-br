...
* Se um bucket tiver o versionamento de objetos ativado, a operação de exclusão de objetos não removerá os marcadores de exclusão criados no StorageGRID 11.7 ou anterior. Consulte informações sobre como excluir objetos em um bucket em link:../ilm/how-objects-and-delete-markers-are-deleted.html#delete-s3-versioned-objects["Como objetos com versão S3 são excluídos"].
...
+
image:delete_bucket_objects_in_progress.png["captura de tela mostrando objetos de bucket sendo excluídos"]
...
+
image:delete_bucket_objects_complete.png["captura de tela mostrando todos os objetos bucket excluídos"]
