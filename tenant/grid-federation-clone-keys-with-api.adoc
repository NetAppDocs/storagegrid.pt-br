...
summary: Se a sua conta de locatário do StorageGRID tiver a permissão Use grid federation connection, você pode usar a Tenant Management API para clonar manualmente as chaves de acesso S3. 
---
...
+
image:grid_federation_post_current_user_replicate.png["POST endpoint para replicar a própria chave de acesso"]
...
+
image:grid_federation_clone_access_key.png["API do Gerenciador de inquilinos inserindo valores para clonar a chave de acesso"]
...
+
image:grid_federation_post_other_user.png["POST endpoint para replicar a chave de acesso de outro usuário"]
