...
summary: 'O firewall interno do StorageGRID permite apenas conexões de entrada para portas específicas na Grid Network, com exceção das portas 22, 80, 123 e 443 (consulte as informações sobre comunicações externas). Conexões também são aceitas em portas definidas pelos endpoints do balanceador de carga.' 
---
...
Aparelhos
 a| 
Nó de administração primário e nó de administração não primário
 a| 
