...
Os pools de storage de nuvem também exigem acesso dos nós de armazenamento aos endpoints fornecidos pelo serviço externo utilizado, como Amazon S3 Glacier ou Microsoft Azure Blob Storage. Para obter informações, consulte link:../ilm/what-cloud-storage-pool-is.html["O que é um Cloud Storage Pool"].
