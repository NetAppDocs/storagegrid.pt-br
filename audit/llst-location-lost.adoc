...
summary: 'Esta mensagem é gerada sempre que não for possível encontrar a localização de uma cópia de um objeto (replicada ou codificada para apagamento).' 
---
= Mensagem de auditoria LLST para locais de cópia de objetos perdidos no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
