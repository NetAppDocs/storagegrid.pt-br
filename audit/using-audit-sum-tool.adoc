...
summary: 'Você pode usar a ferramenta audit-sum para contar as mensagens de auditoria de gravação, leitura, cabeçalho e exclusão e para ver o tempo (ou tamanho) mínimo, máximo e médio para cada tipo de operação.' 
---
= Use a ferramenta audit-sum para resumir as mensagens de auditoria do StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
