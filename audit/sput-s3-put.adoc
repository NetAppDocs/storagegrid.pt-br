...
---
= Mensagem de auditoria SPUT para transações PUT do S3 no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
ASRS
 a| 
URN da sessão de função assumida
 a| 
O identificador único da sessão de função assumida.

Presente somente quando a solicitação S3 está usando credenciais de segurança temporárias de uma solicitação AssumeRole.
...
ASRU
 a| 
URN de função assumida
 a| 
O identificador único da função assumida.
