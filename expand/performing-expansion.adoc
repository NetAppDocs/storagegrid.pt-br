...
---
= Realizar uma expansão do StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Passos
. Selecione *Manutenção* > *Tarefas* > *Expansão da grid*.
+
A página de expansão da grid é exibida. A seção Nós Pendentes lista os nós que estão prontos para serem adicionados.
...
+
image:grid_node_details.png["Captura de tela dos detalhes do Grid Node"]
...
. Quando você tiver aprovado todos os nós da grade, insira a *senha de provisionamento* e selecione *Expand*.
+
