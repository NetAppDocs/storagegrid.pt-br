...
* Você confirmou que a regra padrão na política ILM ativa está em conformidade. Consulte link:creating-default-ilm-rule.html["Crie uma regra ILM padrão"] para obter detalhes.
...
** Se a regra padrão não for compatível, um erro será exibido. Você deve criar e ativar uma nova política ILM que inclua uma regra compatível como regra padrão. Selecione *OK*. Em seguida, crie uma nova política, simule-a e ative-a. Consulte link:creating-ilm-policy.html["Criar política ILM"] para obter instruções.
