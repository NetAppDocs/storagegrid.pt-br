...
---
= Crie políticas ILM no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Se o link:enabling-s3-object-lock.html["A definição Global S3 Object Lock foi ativada"], você deve garantir que a política ILM esteja em conformidade com os requisitos dos buckets que têm o S3 Object Lock ativado. Nesta seção, siga as instruções que mencionam ter o S3 Object Lock ativado.
...
--
** O link:enabling-s3-object-lock.html["A definição Global S3 Object Lock já está ativada"] para o sistema StorageGRID.
...
.Resultados
Quaisquer alterações nas regras da política serão refletidas nos resultados da simulação e mostrarão a nova correspondência e a correspondência anterior. A janela "Simular política" mantém os objetos testados até que você selecione "Limpar tudo" ou o ícone de remoção image:icon_x_to_remove.png["Remover ícone"] para cada objeto na lista de resultados da simulação.
