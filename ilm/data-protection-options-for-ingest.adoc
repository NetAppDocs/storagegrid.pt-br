...
summary: Ao criar uma regra ILM, você pode proteger objetos na ingestão especificando Dual commit, Strict ou Balanced. 
---
= Opções de ingestão de ILM no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:ingest_object_lifecycle.png["Diagrama mostrando como ocorre a ingestão de objetos para cada opção de ingestão"]
