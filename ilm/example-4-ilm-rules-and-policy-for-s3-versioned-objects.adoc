...
---
= Regras e políticas de ILM para versões não atuais de objetos S3 no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:ilm_rule_apply_only_to_older_object_verions.png["Aplicar regra ILM apenas a versões de objetos mais antigas (tempo não atual)"]
...
link:how-objects-and-delete-markers-are-deleted.html["Como os objetos e os marcadores de exclusão são excluídos"]
