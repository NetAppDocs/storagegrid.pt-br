...
summary: 'Para gerenciar objetos, você cria um conjunto de regras de gerenciamento do ciclo de vida das informações (ILM) e as organiza em uma política ILM.' 
---
= Saiba mais sobre as regras ILM no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:ilm_rule_multiple_placements_in_single_time_period.png["Tela de posicionamentos ILM do Gerenciador de Grade"]

A primeira instrução de colocação image:icon_number_1.png["Ícone número 1"] tem duas linhas para o primeiro ano:
...
A segunda instrução de colocação image:icon_number_2.png["Ícone número 1"] cria duas cópias após um ano e guarda essas cópias para sempre.
