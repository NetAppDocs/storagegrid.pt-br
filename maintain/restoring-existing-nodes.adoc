...
Se você recuperar um nó de um nó de administração não primário, defina ADMIN_IP para o endereço IP da Rede de Grade do nó de administração não primário, para que o nó recuperado se registre no nó de administração não primário em vez do nó de administração primário. Para visualizar as regras para chaves de Rede de Administração, consulte link:../swnodes/creating-node-configuration-files.html["Crie arquivos de configuração de nós do StorageGRID para implantações Linux"].
