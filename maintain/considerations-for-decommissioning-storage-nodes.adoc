...
---
= Considerações sobre a desativação de Storage Node no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* O sistema deve, em todos os momentos, incluir nós de armazenamento suficientes para atender aos requisitos operacionais, incluindo o link:understanding-adc-service-quorum.html["Quórum de ADC"] e o link:reviewing-ilm-policy-and-storage-configuration.html["Política de ILM"] ativo. Se uma desativação reduzir a contagem de ADC de um site abaixo do quórum, o StorageGRID move automaticamente o serviço de ADC para nós de armazenamento disponíveis que não sejam ADC no mesmo site, mantendo o quórum de ADC. Se o serviço de ADC não puder ser movido automaticamente, talvez você precise link:../expand/adding-grid-nodes-to-existing-site-or-adding-new-site.html["Adicionar um novo Storage Node em uma operação de expansão"] antes de desativar um nó de armazenamento existente.
+
