...
---
= Adicionar ou atualizar nomes de exibição do StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Para remover os itens que você não deseja renomear, selecione image:icon-x-to-remove.png["Ícone X."] na coluna *Remover da lista*.
+
...
. Selecione image:icon-x-to-remove.png["Ícone X."] para remover quaisquer itens que você não queira renomear com esta ferramenta.
+
Por exemplo, suponha que você queira renomear todos os nós que contenham a string  `Data-Center-1`, mas não queira renomear o site  `Data-Center-1` em si. Selecione image:icon-x-to-remove.png["Ícone X."] para remover o site da pré-visualização de renomeação.

+
image:rename-bulk-rename-tool.png["Ferramenta de renomeação em massa antes"]
...
+
image:rename-bulk-rename-tool-after.png["Renomeie a ferramenta em massa depois"]
...
. Faça as alterações adicionais necessárias ou selecione image:icon-x-to-remove.png["Ícone X."] para remover os itens que você não quer renomear.
...
--
.. Copie o nome original de cada item no sistema e cole-o no campo *Nome de exibição*, ou selecione image:icon-x-to-remove.png["Ícone X."] para remover os itens que você não deseja reverter.
+
...
.. Confirme se cada entrada no campo *Nome de exibição* corresponde ao nome no campo *Nome do sistema*. Faça as alterações desejadas ou selecione image:icon-x-to-remove.png["Ícone X."] para remover os itens que você não deseja reverter.
+
Para reverter um nome de exibição, o nome do sistema deve aparecer no campo *Nome de exibição*, mas o nome não diferencia maiúsculas de minúsculas.
