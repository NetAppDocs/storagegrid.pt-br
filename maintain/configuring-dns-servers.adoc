...
summary: 'Você pode adicionar, remover e atualizar servidores DNS, para que você possa usar nomes de host de domínio totalmente qualificado (FQDN) em vez de endereços IP.' 
---
= Configurar os servidores DNS no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Passos
. Selecione *Manutenção* > *Rede* > *Servidores DNS*.
. Selecione o ícone de exclusão image:icon-x-to-remove.png["eliminar ícone"] ao lado do endereço IP.
. Selecione *Guardar*.
