...
---
= Execute uma reinicialização gradual do StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
TIP: Não é possível reinicializar um nó se ele estiver desconectado do grid. As caixas de seleção estão desabilitadas para nós com estes ícones: image:icon_alarm_gray_administratively_down.png["ícone de ponto de interrogação cinzento"] ou image:icon_alarm_blue_unknown.png["ícone de ponto de interrogação azul"].
