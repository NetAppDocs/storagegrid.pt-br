...
---
= Selecione um site para desativação no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image:decommission_site_step_select_site.png["Etapa 1 Selecione o local"]
...
| Desativar possível motivo | Descrição | Próximo passo 
 ...
 a| 
Marca de verificação verde (image:icon_alert_green_checkmark.png["Marca de verificação Icon Alert Green"])
 a| 
...
Não. Todos os nós neste local estão desconetados. Contacte o representante da sua conta NetApp.
 a| 
Você não pode realizar a desativação de um site conectado a menos que todos os nós do site estejam conectados (image:icon_alert_green_checkmark.png["Marca de verificação Icon Alert Green"]).
 a| 
...
|===
+
O exemplo mostra um sistema StorageGRID com três sites. A marca de seleção verde (image:icon_alert_green_checkmark.png["Marca de verificação Icon Alert Green"]) para os sites de Raleigh e Sunnyvale indica que você pode descomissionar esses sites. No entanto, você não pode descomissionar o site de Vancouver porque ele contém o nó de administração principal.
