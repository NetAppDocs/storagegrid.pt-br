...
---
= Inicie a instalação do software StorageGRID em um appliance de serviços
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Se você estiver instalando um Nó de Gateway ou um Nó de Administração não primário, você conhece o endereço IP de um dos seguintes Nós de Administração:
+
** O nó de administração principal para a grade StorageGRID
** Um nó Admin não primário, se houver um disponível e se estiver sendo usado para recuperar o nó


* Se você estiver recuperando um nó (incluindo o Nó de Administração Primário) usando um Nó de Administração não primário, poderá especificar o endereço IP desse Nó de Administração.
...
* Para um Nó de Administração primário, você especifica o nome do nó e, em seguida, carrega os pacotes de software apropriados (se necessário). Se você estiver recuperando o Nó de Administração primário e tiver um Nó de Administração não primário em sua grid, você tem a opção de recuperar o nó usando um Nó de Administração não primário e especificando o endereço IP desse Nó de Administração ou pode instalar ou recuperar o nó usando sua configuração atual.
...
.. No campo *Nome do nó*, insira o mesmo nome que foi usado para o nó que você está recuperando e selecione *Salvar*.
.. Selecione um modo apropriado dentre os dois disponíveis na seção Modo de Instalação. Você pode selecionar o método Instalar um nó ou recuperar este nó usando a configuração atual ou o método Recuperar este nó usando um nó de administração não primário.
+
[role="tabbed-block"]
====
.Método de configuração atual
--
Instale um nó ou recupere este nó usando a configuração atual.

... Na seção Instalação, verifique a versão do software listada em Estado atual. Se a versão do software pronta para instalar estiver correta, pule para a <<installation_section_step,Etapa de instalação>>.
... Se você precisar carregar uma versão diferente do software, no menu *Avançado*, selecione *Upload StorageGRID software*. A página Upload StorageGRID Software será exibida.
... Selecione *Procurar* para fazer upload do *pacote de software* e do *arquivo de checksum* do StorageGRID. Os arquivos são carregados automaticamente após você selecioná-los.
... Selecione *Início* para retornar à página inicial do instalador do dispositivo StorageGRID.


--
.Método de nó Admin não primário
--
Recupere este nó usando um nó de administração não primário.

... No campo "IP do nó de administração não primário", insira o endereço IP da rede de grid do nó de administração não primário que você deseja usar para a recuperação e selecione *Salvar*.
...
+
Se o botão *Iniciar instalação* não estiver habilitado, talvez seja necessário alterar a configuração de rede ou as configurações de porta.  Consulte as instruções de manutenção do seu aparelho.

. Na página inicial do instalador do StorageGRID Appliance, selecione *Iniciar instalação*.
+
O estado atual muda para "A instalação está em andamento" e a página de instalação do monitor é exibida.

+

NOTE: Se você precisar acessar a página de instalação do Monitor manualmente, selecione *Monitor installation* na barra de menu.
