...
---
= Alterar os endereços IP de todos os nós em uma grid StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Se o SSO estiver habilitado para o seu sistema StorageGRID e quaisquer relações de confiança com terceiros tiverem sido configuradas usando endereços IP do nó de administração (em vez de nomes de domínio totalmente qualificados, conforme recomendado), prepare-se para atualizar ou reconfigurar essas relações de confiança com terceiros no Active Directory Federation Services (AD FS) imediatamente após você alterar os endereços IP. Consulte configurar o SSO usando link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] ou link:../admin/learn-about-setting-up-sso-saml.html["SAML"].
...
. Se você permitiu acesso SSH externo,link:../admin/manage-external-ssh-access.html["bloquear acesso"] quando terminar de alterar os endereços IP.
