...
NOTE: Se o SSO estiver habilitado para seu sistema StorageGRID e quaisquer trusts de terceira parte confiável tiverem sido configurados usando endereços IP do nó de administração (em vez de nomes de domínio totalmente qualificados, conforme recomendado), esteja preparado para atualizar ou reconfigurar esses trusts de terceira parte confiável nos Serviços de Federação do Active Directory (AD FS) imediatamente após alterar os endereços IP. Ver link:../admin/configure-sso.html["Configurar o logon único"] .
