...
---
= Desligue e ligue todos os nós do StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Ao desligar todos os nós em um site ou grid, você não poderá acessar os objetos ingeridos enquanto os Storage Nodes estiverem offline.
...
.Sobre esta tarefa
Primeiro, pare os serviços nos nós de administração e nos nós de gateway, depois pare os serviços nos nós de armazenamento.
...
NOTE: Se um host incluir vários nós de grid, os nós voltam a ficar online automaticamente quando você liga o host.

.Passos
. Ligue os hosts para o nó de administração principal e quaisquer nós de administração não primários.
+

NOTE: Você não pode fazer login nos Nós de Administração até que os Nós de Armazenamento sejam reiniciados.
