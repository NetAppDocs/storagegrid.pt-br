...
summary: Você pode alterar a configuração de rede de um ou mais nós usando a ferramenta Alterar IP. Você pode alterar a configuração da Grid Network ou adicionar, alterar ou remover as Admin ou Client Networks. Você também pode alterar as configurações de IP da LAN para um nó de appliance BMC. 
---
= Alterar a configuração de rede do nó no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Você pode alterar a configuração de rede de um ou mais nós usando a ferramenta Alterar IP. Você pode alterar a configuração da Grid Network ou adicionar, alterar ou remover as Admin ou Client Networks. Você também pode alterar as configurações de IP da LAN para um nó de appliance BMC.
...
NOTE: Se o logon único (SSO) estiver habilitado para o seu sistema StorageGRID e você estiver alterando o endereço IP de um Nó de Administração, esteja ciente de que qualquer relação de confiança com terceiros configurada usando o endereço IP do Nó de Administração (em vez do seu domínio totalmente qualificado, conforme recomendado) se tornará inválida. Você não poderá mais fazer logon no nó. Imediatamente após alterar o endereço IP, você deve atualizar ou reconfigurar a relação de confiança com terceiros do nó no Active Directory Federation Services (AD FS) com o novo endereço IP. Consulte as instruções para configurar o SSO usando link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] ou link:../admin/learn-about-setting-up-sso-saml.html["SAML"].
...
+
image:change_ip_tool_main_menu.png["Captura de tela mostrando a tela de boas-vindas da ferramenta Change IP"]
...
+
image:change_ip_tool_edit_ip_mask_sample_output.png["captura de tela descrita pelo texto circundante"]
...
+
image:change_ip_tool_validate_sample_error_messages.gif["captura de tela descrita pelo texto circundante"]

+
Neste exemplo, a validação passou.

+
image:change_ip_tool_validate_sample_passed_messages.gif["captura de tela descrita pelo texto circundante"]
...
== Alterar as configurações de endereço IP da LAN do BMC do nó do appliance

Você pode alterar as configurações de IP da LAN para um nó de appliance BMC.

. No Grid Manager, vá para *Nodes* > *_grid node_* > *BMC settings*.
. Na seção de configurações de IP da LAN, selecione um tipo de atribuição de IP como *Estático* ou *DHCP*.
. Insira o endereço MAC, o endereço IPv4 (CIDR) e o gateway padrão para o BMC.
. Selecione *Guardar*.


Você também pode alterar ou visualizar as configurações de endereço IP da LAN do BMC usando a  `PUT /bmc-configuration/root-password` ou  `GET /bmc-configuration/root-password` API.
