...
---
= Avisos para recuperação da unidade de sistema do nó de armazenamento StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
CAUTION: Se este Nó de Armazenamento estiver em modo de manutenção somente leitura para permitir a recuperação de objetos por outro Nó de Armazenamento com volumes de armazenamento com falha, recupere os volumes no Nó de Armazenamento com falha antes de recuperar este Nó de Armazenamento com falha. Veja as instruções para link:reviewing-warnings-about-storage-volume-recovery.html["recuperar de uma falha no volume de armazenamento em que a unidade do sistema está intacta"].
