...
** Em casos raros, como quando instruído pelo suporte técnico ou quando você souber que o nó de substituição tem menos volumes disponíveis para storage de objetos do que o nó original, você deve link:restoring-object-data-to-storage-volume-for-appliance.html["restaure os dados do objeto manualmente"] usar o `repair-data` script. Se um desses casos se aplicar, responda `n`.
+
[NOTE]
====
