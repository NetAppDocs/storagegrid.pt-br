...
summary: 'Na Etapa 5 (resolver conflitos de nós) do assistente do local de desativação, você pode determinar se algum nó no sistema StorageGRID está desconetado ou se algum nó no local selecionado pertence a um grupo de alta disponibilidade (HA). Depois que qualquer conflito de nó for resolvido, você inicia o procedimento de desativação nesta página.' 
---
= Resolva conflitos de nós e inicie a desativação do site no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Todos os nós do seu sistema StorageGRID devem estar conectados (image:icon_alert_green_checkmark.png["Marca de verificação Icon Alert Green"]).
+
...
.Passos
. Revise a seção de nós desconectados da Etapa 5 (Resolver conflitos de nós) para determinar se algum nó em seu StorageGRID tem um estado de conexão Desconhecido (image:icon_alarm_blue_unknown.png["Ícone alarme azul desconhecido"]) ou Administrativamente Inativo (image:icon_alarm_gray_administratively_down.png["Ícone alarme cinza administrativamente para baixo"]).
+
image:decommission_site_step_5_disconnected_nodes.png["Etapa 5 resolver conflitos de nó"]
...
+
image:decommission_site_step_5_ha_groups.png["Desativar os grupos de HA do passo 5 do local"]
...
+
image:decommission_site_step_5_provision_passphrase.png["Desativar a frase-passe de provisão do passo 5 do local"]
...
+
image:decommission_site_step_5_warning.png["Aviso de desativação do local passo 5"]
...
+
image:decommission_site_step_5_grid_configuration.png["Desativar a Configuração da grelha do passo 5 do local"]
