...
---
= Configurar um nó de administração primário substituto no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Selecione *Recuperar um nó de administração primário com falha*.
+
image:install_or_recover_primary_admin_node.png["Instale ou recupere o nó de administração principal"]

. Carregue o backup mais recente do pacote de recuperação:
+
.. Selecione *Procurar*.
.. Localize o arquivo de pacote de recuperação mais recente para o seu sistema StorageGRID e selecione *Abrir*.
...
NOTE: Para configurar uma relação de confiança com uma terceira parte, consulte como configurar SSO usando link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] ou link:../admin/learn-about-setting-up-sso-saml.html["SAML"]. Para acessar o certificado padrão do servidor, faça login no shell de comando do Admin Node. Vá até o diretório  `/var/local/mgmt-api` e selecione o arquivo  `server.crt`.
