...
---
= Acesse a página Decommission Nodes no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
Neste exemplo, uma marca de seleção verde na coluna "Desativação Possível" indica que você pode desativar o Nó do Gateway de API e o Nó de Armazenamento somente de dados. Os outros nós não podem ser desativados por vários motivos. Por exemplo, o Nó de Administração principal não pode ser desativado porque a desativação desse tipo de nó não é suportada. O Nó de Armazenamento somente de metadados não pode ser desativado porque o site deixaria de atender aos requisitos link:../maintain/understanding-adc-service-quorum.html["Quórum de ADC"].

+
image:decommission_nodes_page.png["Desativar a página de nós"]
...
** image:icon_alarm_gray_administratively_down.png["ícone de marca de interrogação cinzenta"] (cinza): Administrativamente inativo
** image:icon_alarm_blue_unknown.png["ícone de ponto de interrogação azul"] (azul): desconhecido
...
Edite o grupo de HA para remover a interface do nó ou remover todo o grupo de HA. link:../admin/configure-high-availability-group.html["Configurar grupos de alta disponibilidade"]Consulte .
 ...
 a| 
Não, o site _x_ precisa de pelo menos 3 nós de armazenamento com serviços ADC ou nós não-ADC disponíveis para mover os serviços ADC. O descomissionamento de nó ADC não está disponível se algum nó estiver offline.
 a| 
*Apenas nós de armazenamento* (nós de armazenamento combinados ou somente de metadados). Você não pode desativar um nó de armazenamento se não restarem nós suficientes no local para atender aos requisitos de quorum do ADC e o serviço ADC não puder ser movido automaticamente. A movimentação automática exige que mais de dois nós de armazenamento ADC estejam online na grid e que haja nós de armazenamento não-ADC disponíveis no mesmo local.
 a| 
Certifique-se de que as condições para a movimentação automática do serviço ADC sejam atendidas (consulte link:../maintain/understanding-adc-service-quorum.html["Quórum de ADC"]), ou link:../maintain/move-adc-service.html["mover manualmente o serviço ADC"] para outro Storage Node no mesmo site. Você também pode realizar uma expansão adicionando um novo Storage Node ao site e especificando que ele deve ter um serviço ADC. Consulte link:../expand/adding-grid-nodes-to-existing-site-or-adding-new-site.html["Resumo do fluxo de trabalho para adicionar nós de grid ao StorageGRID"].
