...
+
O procedimento de desativação é iniciado e o progresso é exibido para cada nó. Durante o procedimento, um novo pacote de recuperação é gerado contendo a alteração da configuração da grade.

. Assim que o novo pacote de recuperação estiver disponível, clique no link ou selecione *MAINTENANCE* > *System* > *Recovery package* para acessar a página do pacote de recuperação. Em seguida, baixe o `.zip` arquivo.
+
