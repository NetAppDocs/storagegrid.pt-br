...
* `Content-Encoding`
+
Ao especificar  `aws-chunked` para  `Content-Encoding`, StorageGRID não verifica os seguintes itens:

+
** StorageGRID não verifica o  `chunk-signature` em relação aos dados do bloco.
...
* Cabeçalhos de solicitação de checksum:
+
--
...
NOTE: Se uma solicitação PutObject não especificar um checksum, o StorageGRID não calculará um checksum CRC-64/NVME padrão para um objeto.
