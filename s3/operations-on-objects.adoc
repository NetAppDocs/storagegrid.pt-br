...
* Os  `If-Match` e  `If-None-Match` cabeçalhos se comportam de maneira diferente dependendo da sua configuração do StorageGRID. Fornecer esses cabeçalhos pode retornar uma mensagem de erro (501 Não Implementado). Se os  `If-Match` ou  `If-None-Match` cabeçalhos forem aceitos, mas não funcionarem, um alerta pode ser acionado com instruções sobre como habilitar o comportamento de erro mais rigoroso de 501 Não Implementado.
