...
NOTE: StorageGRID não suporta o  `x-amz-sdk-checksum-algorithm` cabeçalho de solicitação.
...
* Gerencie as configurações do bucket a partir do Tenant Manager. Consulte link:../tenant/update-default-retention-settings.html["Atualização S3 retenção padrão bloqueio Objeto"].
...
Você deve especificar um dos seguintes cabeçalhos de solicitação na requisição PutObjectLockConfiguration:
...
NOTE: StorageGRID não valida o conteúdo do cabeçalho x-amz-checksum-* para esta operação.



==== Exemplo de solicitação

Este exemplo habilita o S3 Object Lock para um bucket (se o administrador da sua grid tiver concedido permissão) e define o modo de retenção padrão como COMPLIANCE e o período de retenção padrão como 6 anos.
...
Quando novas versões de objetos são ingeridas no bucket, o modo de retenção padrão é aplicado se  `x-amz-object-lock-mode`não for especificado. O período de retenção padrão é usado para calcular a data de retenção (retain-until-date) se  `x-amz-object-lock-retain-until-date`não for especificado.
...
* Você deve incluir um dos seguintes cabeçalhos de solicitação se algum cabeçalho de solicitação x-amz-object-lock-* estiver presente na solicitação PutObject:
+
[listing]
----
Content-MD5
...
----
+
Esses cabeçalhos não são necessários para CopyObject ou CreateMultipartUpload.

* Se o bucket não tiver o S3 Object Lock ativado e um cabeçalho de solicitação x-amz-object-lock-* estiver presente, um erro 400 Bad Request (InvalidRequest) será retornado.
...
Você deve especificar um dos seguintes cabeçalhos de solicitação na solicitação:
...
NOTE: StorageGRID não valida o conteúdo do cabeçalho x-amz-checksum-* para essas operações.
...
+
O  `x-amz-bypass-governance-retention`cabeçalho não tem efeito em objetos sem S3 Object Lock.
