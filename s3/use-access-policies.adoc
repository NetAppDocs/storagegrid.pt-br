...
summary: 'StorageGRID usa a linguagem de políticas da Amazon Web Services (AWS) para permitir que os tenants do S3 controlem o acesso a buckets e objetos dentro desses buckets.' 
---
...
== [[use-putoverwriteobject-permission]]Use a permissão PutOverwriteObject
...
* Quando essa permissão não está presente, o efeito é o mesmo que se Allow estivesse ativado.
...
Operadores IP
 a| 
Compara com o endereço IP de onde a solicitação foi enviada. Pode ser usado para operações de bucket ou objeto.

*Nota:* Se a solicitação S3 foi enviada através do serviço Load Balancer nos nós Admin e nos nós Gateway, isso se compara ao endereço IP upstream do serviço Load Balancer.

*Nota*: Se um balanceador de carga de terceiro não transparente for usado, isso será comparado ao endereço IP desse balanceador de carga. Qualquer  `X-Forwarded-For` cabeçalho será ignorado, pois sua validade não pode ser verificada.
...
Recurso/identidade
 a| 
Compara com o nome de usuário do remetente a partir do qual a solicitação foi enviada. Pode ser usado para operações de bucket ou objeto.
...
S3:ListBucketVersions Permissions
 a| 
Compara com o parâmetro delimitador especificado em uma solicitação ListObjects ou ListObjectVersions.
...
S3:PutObjectVersionTagging
 a| 
Requer que o objeto existente tenha a chave e o valor de tag específicos.
...
S3:ListBucketVersions Permissions
 a| 
Compara com o parâmetro max-keys especificado em uma solicitação ListObjects ou ListObjectVersions.
...
S3:ListBucketVersions Permissions
 a| 
Compara com o parâmetro de prefixo especificado em uma solicitação ListObjects ou ListObjectVersions, ou com a chave especificada para GetObject ou HeadObject.
...
S3:PutObjectVersionTagging
 a| 
Requer uma chave e um valor de tag específicos quando a solicitação do objeto inclui tagging.
...
S3:PutObject
 a| 
Compara-se com o x-amz-copy-source expandido do cabeçalho da solicitação nas solicitações PutObject e UploadPartCopy.
...
NOTE: Configurar DeleteObject como NEGAR em uma política do S3 não impede que o ILM exclua objetos quando existe uma regra como "zero cópias após 30 dias".
