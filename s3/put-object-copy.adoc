...
* `Content-Type`
* `x-amz-checksum-algorithm`: Especifique `CRC32`, `CRC32C`, `CRC64NVME`, ou `SHA256`
+
Ao copiar um objeto, se o objeto de origem tiver um  `checksum` ou incluir  `x-amz-checksum-algorithm`, o objeto copiado terá um checksum FULL_OBJECT.

+
Se o objeto de origem tiver um checksum, mas a solicitação não incluir  `x-amz-checksum-algorithm` ou usar um valor de checksum não suportado, o objeto copiado terá o mesmo tipo de checksum que o objeto de origem. StorageGRID ignora valores  `x-amz-checksum-algorithm` inválidos ou não suportados.

+
Se o objeto de origem não tiver um checksum e não incluir  `x-amz-checksum-algorithm` ou se usar um valor de checksum não suportado, o objeto copiado não terá um checksum.

+

NOTE: Se uma solicitação CopyObject não especificar um checksum e o objeto de origem não tiver um checksum, o StorageGRID não calculará um checksum CRC-64/NVME padrão para o objeto.
