...
summary: O sistema StorageGRID suporta um máximo de 20.000 buckets para cada conta de locatário S3. 
---
...
[role="lead"]
O sistema StorageGRID suporta um máximo de 20.000 buckets para cada conta de locatário S3.

Cada grade pode ter um máximo de 100.000 baldes.

Se vários clientes tiverem até 5.000 buckets cada, cada Storage Node na grade deverá ter no mínimo 64 GB de RAM.
...
Consulte as informações a seguir para obter mais informações:
...
Você pode verificar se as atualizações do último tempo de acesso estão habilitadas ou desabilitadas para buckets individuais. Consulte link:get-bucket-last-access-time-request.html["OBTER último tempo de acesso do Bucket"].
...
*Nota*: buckets de diretório S3 não são suportados.
...
* Você pode incluir o  `x-amz-bucket-object-lock-enabled` cabeçalho da solicitação para criar um bucket com S3 Object Lock ativado. Consulte link:../s3/use-s3-api-for-s3-object-lock.html["Use a API REST do S3 para configurar o bloqueio de objetos do S3"].
+
O S3 Object Lock requer o versionamento de buckets, que é ativado automaticamente quando você cria o bucket com o S3 Object Lock habilitado.

+
Você pode habilitar o S3 Object Lock ao criar o bucket ou posteriormente.
...
DeleteBucketLifecycle
 a| 
Exclui a configuração do ciclo de vida do bucket. Consulte link:create-s3-lifecycle-configuration.html["Crie a configuração do ciclo de vida do S3"].
...
(Anteriormente chamado GET Bucket Lifecycle)
 a| 
Retorna a configuração do ciclo de vida do bucket. Consulte link:create-s3-lifecycle-configuration.html["Crie a configuração do ciclo de vida do S3"].
...
GetBucketlocalização
 a| 
Retorna a região que foi definida usando o  `LocationConstraint` elemento na solicitação CreateBucket. Se a região do bucket for  `us-east-1` (padrão), um valor vazio será retornado para  `LocationConstraint`.
...
*Atenção*: Se uma tag de política ILM não padrão estiver definida para este bucket, haverá uma  `NTAP-SG-ILM-BUCKET-TAG` tag de bucket com um valor atribuído a ela. Não modifique nem remova esta tag.
...
Consulte link:../s3/use-s3-api-for-s3-object-lock.html["Use a API REST do S3 para configurar o bloqueio de objetos do S3"].
...
Consulte link:create-s3-lifecycle-configuration.html["Crie a configuração do ciclo de vida do S3"]. Para entender como a ação de expiração no ciclo de vida do bucket interage com as instruções de posicionamento do ILM, consulte link:../ilm/how-ilm-operates-throughout-objects-life.html["Como o ILM opera ao longo da vida de um objeto"].
...
Política de PutBucketPolicy
 a| 
Define a política associada ao bucket. Consulte link:use-access-policies.html["Use políticas de acesso de grupo e bucket"].
...
* Os buckets de destino devem ser especificados como o URN dos endpoints do StorageGRID, conforme especificado no Tenant Manager ou na API de Gerenciamento de Tenant. Consulte link:../tenant/configuring-cloudmirror-replication.html["Configurar a replicação do CloudMirror"].
+
...
* Se o administrador da grid tiver concedido permissão, habilita o S3 Object Lock nos buckets existentes.
...
Consulte link:../s3/use-s3-api-for-s3-object-lock.html["Use a API REST do S3 para configurar o bloqueio de objetos do S3"] para obter informações detalhadas.
