...
*Importante*: Se um objeto tiver sido migrado para o S3 Glacier Deep Archive ou se o Cloud Storage Pool usar o Azure Blob Storage, você não poderá restaurá-lo usando a camada  `Expedited`. O seguinte erro é retornado `403 Forbidden`, `InvalidTier`: `Retrieval option is not supported by this storage class`.
