...
NOTE: Se a solicitação CreateMultipartUpload não especificar um checksum, o StorageGRID não calcula um checksum CRC-64/NVME padrão para um objeto. StorageGRID ignora valores  `x-amz-checksum-algorithm` inválidos ou não suportados.
