...
Documentação do StorageGRID
 a| 
* https://library.netapp.com/ecm/ecm_download_file/ECMLP3373065["Notas de lançamento"^]. Você precisa fazer login com sua NetApp conta ou criar uma conta para acessar as notas de versão. Leia-as atentamente antes de iniciar a atualização.
* https://kb.netapp.com/hybrid/StorageGRID/Maintenance/StorageGRID_12.1_software_upgrade_resolution_guide["Guia de resolução de atualização do software StorageGRID"^] para a versão principal para a qual você está atualizando (login necessário)
