...
---
= Como as atualizações do StorageGRID afetam seu sistema
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Durante a atualização do StorageGRID 12.1:

* Todos os nós do dispositivo StorageGRID são atualizados automaticamente para a versão 12.1 do firmware do StorageGRID Appliance Installer.
...
* Os dispositivos SGF6112, SG6160, SG110 e SG1100 são atualizados automaticamente para o firmware da BIOS versão 3A14.QD e o firmware do BMC versão 3.21.07.
...
Por exemplo, você pode ver o alerta *Não foi possível comunicar com o nó* quando os serviços são interrompidos, ou o alerta *Erro de comunicação do Cassandra* quando alguns nós foram atualizados para StorageGRID 12.0 ou posterior, mas outros ainda estão executando uma versão anterior do StorageGRID. Em geral, esses alertas desaparecem quando a atualização é concluída.

O alerta *Posicionamento ILM inatingível* pode ser acionado quando os nós de armazenamento são interrompidos durante a atualização para StorageGRID 12.0 ou posterior. Esse alerta pode persistir por 1 dia após a conclusão da atualização.
...
NOTE: As restrições de alteração de configuração são específicas da versão. Se você estiver atualizando para outra versão do StorageGRID, consulte a lista de alterações restritas nas instruções de atualização dessa versão.
...
* Você não deve gerenciar grupos de alta disponibilidade (HA), interfaces VLAN ou endpoints de balanceamento de carga enquanto estiver atualizando.
* Não exclua nenhum grupo HA até que a atualização esteja concluída. Endereços IP virtuais em outros grupos HA podem ficar inacessíveis.
