...
summary: Você pode atualizar para StorageGRID 12.1 e aplicar o hotfix mais recente para essa versão ao mesmo tempo. A página de upgrade do StorageGRID fornece o caminho de atualização recomendado e links diretos para as páginas de download corretas. 
---
= Execute uma atualização do StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Você pode atualizar para StorageGRID 12.1 e aplicar o hotfix mais recente para essa versão ao mesmo tempo. A página de upgrade do StorageGRID fornece o caminho de atualização recomendado e links diretos para as páginas de download corretas.
...
O caminho de atualização na página de atualização do StorageGRID indica quais versões principais (por exemplo, 12.1.0) e hotfixes (por exemplo, 12.1.0.1) você deve instalar para obter a versão mais recente do StorageGRID. Você deve instalar as versões e hotfixes recomendados na ordem apresentada.
...
. Selecione *Procurar* para carregar o arquivo de atualização de versão no StorageGRID: `NetApp_StorageGRID_12.1.0_Software_uniqueID.upgrade`
+
...
. Analise os detalhes de cada verificação prévia relatada e resolva quaisquer erros reportados. Consulte a  https://kb.netapp.com/hybrid/StorageGRID/Maintenance/StorageGRID_12.1_software_upgrade_resolution_guide["Guia de resolução de atualização do software StorageGRID"^] para o lançamento do StorageGRID 12.1.
+
...
NOTE: Para uma atualização do StorageGRID 12.1, se você deseja aprovar nós individuais em vez de todos os nós, a melhor prática é atualizar um site inteiro antes de passar para o próximo site.
...
.Passos
. Assim que a tarefa *Habilitar recursos* for concluída (até uma hora), você pode começar a usar o link:whats-new.html["novas funcionalidades"] na versão atualizada do StorageGRID.
+
