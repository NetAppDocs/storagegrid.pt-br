...
---
= Configure o System Manager do ONTAP para adicionar o StorageGRID como uma camada de nuvem FabricPool
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Estilo de URL
 a| 
Se você link:../admin/configuring-s3-api-endpoint-domain-names.html["Configurados S3 nomes de domínio de endpoint"]selecionar *URL no estilo de hospedagem virtual*.

*URL no formato de caminho* é o padrão para ONTAP, mas o uso de solicitações no formato de hospedagem virtual é recomendado para StorageGRID. Você deve usar *URL no formato de caminho* se fornecer um endereço IP em vez de um nome de domínio no campo *Server name (FQDN)*.
