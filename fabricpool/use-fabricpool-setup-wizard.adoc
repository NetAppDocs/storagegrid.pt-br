...
---
= Saiba mais sobre o assistente de instalação do StorageGRID FabricPool
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Se você planeja usar solicitações no estilo virtual-hosted do S3, você tem link:../admin/configuring-s3-api-endpoint-domain-names.html["Configurados S3 nomes de domínio de endpoint"]. ONTAP usa URLs no estilo caminho por padrão, mas o uso de solicitações no estilo virtual-hosted é recomendado.
