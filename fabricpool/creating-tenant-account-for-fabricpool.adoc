...
---
= Crie uma conta de locatário para FabricPool no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.. Opcionalmente, selecione *Usar conexão de federação de grid* para permitir que o locatário use uma link:../admin/grid-federation-overview.html["conexão de federação de grade"] para sincronização de contas e replicação entre grids. Em seguida, selecione a conexão de federação de grid a ser usada.


. Para a etapa Definir acesso root, especifique qual usuário terá a permissão de acesso root inicial para a conta do locatário, com base no uso do sistema StorageGRIDlink:../admin/using-identity-federation.html["federação de identidade"] ,link:../admin/how-sso-works.html["Logon único (SSO)"] , ou ambos.
+
[cols="1a,2a"]
|===
| Opção | Faça isso 
