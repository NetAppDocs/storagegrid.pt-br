...
---
= Solucionar problemas do nó de administração e da interface de usuário no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Se o login único (SSO) estiver habilitado para o seu sistema StorageGRID, consulte como configurar o SSO usando link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] ou link:../admin/learn-about-setting-up-sso-saml.html["SAML"].
+
...
... Selecione *Test connection* para validar suas configurações de conexão com o servidor LDAP.
...
. Utilize o Secure Shell (SSH) para fazer login no nó de administração:
+
