...
Você pode habilitar ou desabilitar o acesso IPMI remoto para todos os appliances que contêm um BMC. A interface IPMI remota permite o acesso de baixo nível ao hardware dos seus appliances StorageGRID por qualquer pessoa com uma conta e senha do BMC. Se você não precisa de acesso IPMI remoto ao BMC, desabilite esta opção.

* Para controlar o acesso IPMI remoto ao BMC no Grid Manager, acesse *Configuração* > *Segurança* > *Configurações de segurança* > *Dispositivos* > *Acesso IPMI remoto*:
+
...
== Alterar a senha do BMC do appliance

Você pode alterar a senha do BMC de um dispositivo para limitar o acesso e melhorar a segurança do sistema.

. No Grid Manager, vá para *Nodes* > *_grid node_* > *BMC settings*.
. Na seção Alterar senha, insira uma nova senha do BMC com 8 a 16 caracteres em *Nova senha* e, em seguida, insira-a novamente em *Confirmar nova senha*.
. Selecione *Guardar*.


Você também pode alterar a senha do BMC usando a  `PUT /bmc-configuration/root-password`API.



[[control-remote-access-through-usb-portscontrol-external-usb-port-access-on-appliances]]
== [[control-remote-access-through-usb-ports]]Controle o acesso a portas USB externas em appliances

Você pode habilitar as portas USB nos dispositivos para permitir o acesso do suporte técnico para diagnóstico e solução de problemas. Se você não precisar de acesso às portas USB, desabilite as portas por segurança. As portas USB são habilitadas por padrão após a instalação inicial ou uma atualização.

* No Grid Manager, acesse *Configuração* > *Segurança* > *Configurações de segurança* > *Appliances* > *Acesso à porta USB* para ativar ou desativar as portas USB:
+
** Selecione a caixa de seleção *Ativar portas USB* e salve para habilitar as portas.
+
Se a caixa de seleção USB foi desmarcada e salva anteriormente, selecione a caixa de seleção *Ativar portas USB* e salve para ativar as portas USB novamente.

** Desmarque a caixa de seleção *Ativar portas USB* e salve para desativar as portas.
...
* Não permita que clientes não confiáveis se conectem diretamente aos Storage Nodes. Use um endpoint de balanceador de carga fornecido por um Gateway Node ou um load balancer de terceiro.
