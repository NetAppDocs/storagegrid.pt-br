...
---
= Monitor operações de verificação de objetos no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image:nodes_storage_node_object_verification.png["Nós > Nó de armazenamento > Objeto > Verificação"]
...
+
image:nodes_storage_node_ilm_ec_verification.png["Nodes > Storage Node > ILM > Verificação EC"]

+
Selecione o ponto de interrogação image:icon_nms_question.png["ponto de interrogação"] ao lado do nome de um atributo para exibir o texto de ajuda.
...
+
image:oec_job_history.png["Histórico de trabalhos de verificação de existência de objeto"]
