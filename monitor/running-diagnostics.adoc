...
---
= Execute diagnósticos no StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* image:icon_alert_green_checkmark.png["Marca de verificação Icon Alert Green"]*Normal*: Todos os valores estão dentro da faixa normal.
* image:icon_alert_yellow_minor.png["Ícone Alerta Amarelo menor"]*Atenção*: Um ou mais valores estão fora da faixa normal.
* image:icon_alert_red_critical.png["Ícone Alerta Vermelho crítico"]*Atenção*: Um ou mais valores estão significativamente fora da faixa normal.
...
+
image:support_diagnostics_page.png["Página de Diagnóstico de suporte"]

. Para obter mais informações sobre um diagnóstico específico, selecione essa linha.
+
...
+
image:support_diagnostics_cpu_utilization.png["Exemplo de valores de status de diagnóstico"]
...
+
image:grafana_dashboard_nodes.png["Nós do Grafana Dashboard"]

. *Opcional*: Para visualizar um gráfico da expressão do Prometheus ao longo do tempo, selecione *Visualizar no Prometheus*.
+
Aparece um gráfico Prometheus da expressão usada no diagnóstico.

+
image:support_diagnostics_prometheus_png.png["Página Prometheus de Diagnóstico de suporte"]
