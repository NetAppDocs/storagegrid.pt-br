...
Avaria na bateria CMOS do aparelho
 a| 
Foi detectado um problema com a bateria CMOS no appliance.
...
A versão do software SANtricity é inferior ao mínimo recomendado para esta versão do StorageGRID.
 ...
 a| 
Falha detectada no appliance SFP
 a| 
Foi detectada uma falha no SFP no controlador de computação de um appliance StorageGRID.
...
Um bucket do FabricPool tem controle de versão ou bloqueio de objeto S3 habilitado, que não são suportados.
 ...
 a| 
ListBucket verificação de permissão para objeto Get/Head desativada
 a| 
ListBucket a verificação de permissões para GetObject/HeadObject em chaves inexistentes está desativada.
...
| Nome do alerta | Descrição 
 ...
 a| 
Serviço de chunks com alta contagem de goroutines
 a| 
O serviço de fragmentação apresenta um número excepcionalmente alto de goroutines, o que pode indicar um deadlock ou vazamento de recursos.
...
Fragmentos codificados por apagamento não podem mais ser verificados. Fragmentos corrompidos podem não ser reparados.
 ...
 a| 
Grupos codificados por apagamento no estado Needs Repair
 a| 
Um ou mais grupos codificados para apagamento estão no estado "Necessita de Reparo" quando nenhum reparo de dados codificados para apagamento está em andamento.
...
Objetos órfãos foram detectados.
 ...
 a| 
O tamanho da parte do upload multipart do S3 é muito grande
 a| 
Um cliente está tentando realizar uma operação de upload multipart que excede o limite de tamanho da parte.
...
Um cliente está tentando uma operação PUT Object que excede os limites de tamanho S3.
 ...
 a| 
Requisições S3 usando cabeçalhos condicionais não suportados
 a| 
Um cliente S3 está usando cabeçalhos condicionais não suportados (If-Match ou If-None-Match) para determinadas operações.



 a| 
Solicitações S3 usando região incorreta com namespace global ativado
 a| 
Um cliente S3 está usando a região de assinatura incorreta para acessar os buckets deste grid.
