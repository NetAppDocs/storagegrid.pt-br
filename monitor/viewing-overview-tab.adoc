...
---
= Veja a guia Visão geral do StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:nodes_page_overview_tab.png["Guia Visão geral da página de nós"]
...
+
** *Desconhecido* image:icon_alarm_blue_unknown.png["ícone de ponto de interrogação azul"]: Por algum motivo desconhecido, o nó não está conectado à rede ou um ou mais serviços estão inesperadamente indisponíveis. Por exemplo, a conexão de rede entre os nós foi perdida, a energia caiu ou um serviço está inativo. O alerta *Unable to communicate with node* também pode ser acionado. Outros alertas também podem estar ativos. Esta situação requer atenção imediata.
+
...
** *Administrativamente inativo* image:icon_alarm_gray_administratively_down.png["ícone de marca de interrogação cinzenta"]: o nó não está conectado à grid por um motivo esperado. Por exemplo, o nó, ou os serviços no nó, foi desligado corretamente, o nó está reiniciando ou o software está sendo atualizado. Um ou mais alertas também podem estar ativos.
** *Conectado* image:icon_alert_green_checkmark.png["marca de verificação verde de alerta do ícone"]: o nó está conectado à rede.
...
* *Endereços IP*: Os endereços IP do nó. Selecione *Mostrar endereços IP adicionais* para visualizar os endereços IPv4 e IPv6 do nó e os mapeamentos de interface.
...
image:nodes_page_alerts_table.png["Tabela de alertas de página de nós"]
