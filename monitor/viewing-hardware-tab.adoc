...
---
= Veja a guia Hardware do StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:nodes_page_hardware_tab_graphs.png["Guia hardware da página de nós"]
...
image:nodes_page_memory_usage_details.png["Página de nós > Hardware > Detalhes de uso de memória"]
...
+
image:nodes_page_overview_tab_extended.png["Visão geral da página dos nós estendida"]
...
O status da temperatura do controlador de computação.
 ...
 a| 
Fonte de alimentação do controlador de computação A
 a| 
O status da fonte de alimentação A para o controlador de computação.



 a| 
Fonte de alimentação do controlador de computação B
 a| 
O status da fonte de alimentação B para o controlador de computação.
...
+
image:nodes_page_network_traffic_graph.png["Gráfico de tráfego de rede da página de nós"]

+
.. Revise a seção Interfaces de rede.
+
image:nodes_page_network_interfaces.png["Interfaces de rede da página de nós"]

+
Utilize a tabela a seguir com os valores da coluna *Velocidade* na tabela Interfaces de rede para determinar se as portas de rede 10/25-GbE do appliance foram configuradas para usar o modo ativo/backup ou o modo LACP.
...
.. Analise a seção de comunicação de rede.
+
...
+
image:nodes_page_storage_used_object_data.png["Armazenamento usado - dados do objeto"]

+
image:storage_used_object_metadata.png["Armazenamento usado - metadados Objeto"]
...
+
image:nodes_page_overview_tab_services_appliance.png["Guia Visão geral da página de nós para o dispositivo de serviços"]
...
. Selecione a guia *Hardware* para ver mais informações sobre o appliance.
+
...
+
image:nodes_page_hardware_tab_graphs_services_appliance.png["Gráficos da guia hardware da página de nós para o dispositivo de serviços"]

.. Deslize a tela para baixo para visualizar a tabela de componentes do appliance. A tabela contém informações como o nome do modelo, o número de série, a versão do firmware do controlador e o status de cada componente.
+
[cols="1a,2a"]
|===
| Campo na mesa do aparelho | Descrição 
...
. Selecione a aba *Rede* para visualizar informações de cada rede.
+
O gráfico tráfego de rede fornece um resumo do tráfego de rede geral.
...
.. Revise a seção Interfaces de rede.
+
Utilize a tabela a seguir com os valores da coluna *Velocidade* na tabela Interfaces de rede para determinar se as quatro portas de rede 40/100-GbE do appliance foram configuradas para usar o modo ativo/backup ou o modo LACP.
...
. Selecione a guia *Armazenamento* para visualizar informações sobre os dispositivos de disco e volumes no appliance de serviços.
