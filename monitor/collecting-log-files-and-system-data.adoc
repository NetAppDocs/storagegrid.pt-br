...
---
= Coletar arquivos de log do StorageGRID e dados do sistema
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Se o nó de administração usado para iniciar a coleta de logs ficar offline durante a coleta, StorageGRID não reterá os resultados. Você precisa reinicializar o nó e reiniciar a coleta de logs após a reinicialização ser concluída.
...
NOTE: Você pode optar por deixar o campo Senha de provisionamento vazio para uma nova tentativa padrão ou pode inserir a senha para tentar novamente usando SSH.
