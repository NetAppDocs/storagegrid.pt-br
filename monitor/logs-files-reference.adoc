...
---
= Arquivos de log do StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
| acct-tunnel.log  a| 
Registro do auxiliar de túnel da conta que funciona com o balanceador de carga.
...
Registro de tráfego interno do proxy HTTP.
...
| manage-audit.log  a| 
`mgmt-api`acessar registros de auditoria.
...
* stream-access.log: Sessões de proxy de stream TCP/TLS (camada 4) para o bloco de stream.
+
*Nota*: Isto não é um log de requisições HTTP.
...
Registro de acesso para tráfego de federação de grid roteado como S3 (global namespace/URLs de federação).
...
| system-bucket.log  a| 
`update-system-bucket-configuration.log`: Registros do bucket do sistema.
...
`update-cors-configurations.log`: Registra logs relacionados a alterações de TLS/cipher e CORS.



| update-leakd-configurations.log  a| 
Registro de configuração de detecção de órfãos.
