...
image:storagegrid_system_diagram.png["Imagem com vários clientes"]
...
* Suporte para implantação baseada em software ou baseada em hardware. Você pode implantar StorageGRID em qualquer um dos seguintes:
+
...
* Gerenciamento de identidade federada. Integra-se com Active Directory, OpenLDAP ou Oracle Directory Service para autenticação de usuário. Suporta logon único (SSO) usando Security Assertion Markup Language 2.0 (SAML 2.0) ou OpenID Connect (OIDC) para trocar dados de autenticação e autorização entre StorageGRID e um provedor de identidade externo, como Active Directory Federation Services (AD FS).
