...
---
= Como StorageGRID gerencia cópias de objetos
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:replication_data_flow.png["diagrama que mostra o processo descrito no texto circundante"]
...
image:erasure_coding_data_flow.png["Fluxo de dados de codificação de apagamento"]
...
Se as instruções de posicionamento de conteúdo de uma regra ILM exigirem que uma cópia replicada dos dados do objeto seja armazenada em um Cloud Storage Pool, os dados do objeto serão duplicados para o bucket S3 externo ou para o contêiner Azure Blob Storage especificado para o Cloud Storage Pool.
...
image:cloud_storage_pool_data_flow.png["Fluxo de dados de pool de storage de nuvem"]
